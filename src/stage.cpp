...
void CompleteStage::render() {
    ...
    const int soi_y = rect.h / ( tile_height * 2 );
    const int soi_x = rect.w / ( tile_width * 2 );

    const int toi_y = m_y / tile_height;
    const int too_y = m_y % tile_height;
    const int toi_x = m_x / tile_width;
    const int too_x = m_x % tile_width;
    ...
    const int end_row_px = start_row_px + rect.h + tile_height - 1;
    const int end_col_px = start_col_px + rect.w + tile_width - 1;

    const int max_height = m_height - 1;
    const int max_width = m_width - 1;

    for ( int y = start_row_px, yi = 0; y < end_row_px; y += tile_height, ++yi ) {
        int yn = clamp( 0, yi + toi_y - soi_y, max_height );
        for ( int x = start_col_px, xi = 0; x < end_col_px; x += tile_width, ++xi ) {
            int xn = clamp( 0, xi + toi_x - soi_x, max_width );
            mp_sprite->render(m_data[yn * m_width + xn], x - too_x, y - too_y, SDL_FLIP_NONE);
        }
    }
}
